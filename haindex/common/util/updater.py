...
class RepositoryUpdater(object):
    ...
    def update(self, repository):
        ...
        if not repository.type:
            file_counter = FileCounter(repo=repo)
            file_counter.count(contents=contents)

            if file_counter.get_count('.js') > 0 or file_counter.get_count('.py') > 0:
                if file_counter.get_count('.js') > file_counter.get_count('.py'):
